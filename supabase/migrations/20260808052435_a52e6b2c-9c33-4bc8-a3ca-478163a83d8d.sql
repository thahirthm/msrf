...
GRANT SELECT, UPDATE ON public.payment_submissions TO authenticated;
GRANT ALL ON public.payment_submissions TO service_role;
ALTER TABLE public.payment_submissions ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Anyone can submit payment proof" ON public.payment_submissions
FOR INSERT TO anon, authenticated WITH CHECK (status = 'pending' AND rejection_reason IS NULL AND verified_by IS NULL);

CREATE POLICY "Admins can view submissions" ON public.payment_submissions
